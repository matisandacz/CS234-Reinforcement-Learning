...
class Linear(DQN):
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        #Load state into placeholder.
        #Compute linear combination for every input state and for every action.
        with tf.variable_scope(scope, reuse=reuse):
            x = tf.layers.flatten(state) #shape=(batch_size, height*width*channels*state_history) 
            out = tf.layers.dense(x, units=num_actions, use_bias=True)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        op = [tf.assign(target_w[i], w[i]) for i in range(len(w))] #List of tensors from variable nodes.
        self.update_target_op = tf.group(*op) #Operation that executes all tensors.
        ##############################################################
        ######################## END YOUR CODE #######################
